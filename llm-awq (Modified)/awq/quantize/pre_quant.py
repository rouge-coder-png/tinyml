...
def run_awq(
    ...
    for i in tqdm.tqdm(range(len(layers)), desc="Running AWQ..."):
        ...
        #print(f"Line 117, named_linears = {named_linears}")
        ...
        #print(f"Line 126, input_feat = {input_feat}")
        handles = []
        for name in named_linears:
            handles.append(named_linears[name].register_forward_hook(
                functools.partial(cache_input_hook, name=name,
                                  feat_dict=input_feat)))
        #print(f"Line 132, input_feat = {input_feat}")
        inps = inps.to(next(layer.parameters()).device)  # in case multi-gpu
        #print(f"Line 134, input_feat = {input_feat}")
        # get output as next layer's input
        inps = layer(inps, **layer_kwargs)[0]
        #print(f"Line 136, input_feat = {input_feat}")
        for h in handles:
            h.remove()
        # now solve for scaling and clipping
        #print(f"Line 137, input_feat = {input_feat}")
        input_feat = {k: torch.cat(v, dim=0) for k, v in input_feat.items()}
        #print(f"Line 139, input_feat = {input_feat}")
        # Clear GPU memory
        torch.cuda.empty_cache()

        if auto_scale:  # if it applies, we should also modify the input_feat with scales
            #print(f"Line 144, input_feat = {input_feat}")
